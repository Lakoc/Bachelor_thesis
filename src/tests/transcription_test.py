...
def calculate_success_rate():
    with open(f'{params.output_folder}/diarization', 'r') as f1:
        with open(params.transcription_path, 'r') as reference_file:
            ...
            max_val = max(lines11[len(lines11) - 1][1], int(float(reference[len(reference) - 1][2]) * 100))
            ...
            for line in reference:
                from_t, to_t, value = extract_transcribed(line)
                reference_val[np.max([from_t - collar_one_side, 0]): from_t + collar_one_side] = -1
                reference_val[from_t + collar_one_side: to_t + 1 - collar_one_side] = value if value < 3 else -1
                reference_val[to_t + 1 - collar_one_side: np.min([to_t + 1 + collar_one_side, max_val])] = -1
            ...
            print(
                f'False alarm: {(false_alarm / reference_val.shape[0]):.3f} (Marked as active, although speech was '
                f'not present)')
            print(
                f'Confusion: {(confusion / reference_val.shape[0]):.3f} (System provided speaker tag and the '
