...
m = np.loadtxt("features/UBM_means.txt", dtype=np.float32)
iE = 1.0 / np.loadtxt("features/UBM_vars.txt", dtype=np.float32)
w = np.loadtxt("features/UBM_weights.txt", dtype=np.float32)
V = np.loadtxt("features/iXtractor.txt", dtype=np.float32).reshape(-1, *m.shape)

# load MFCC features
X = np.loadtxt("features/same_room_session1/features.txt")
...
q, sp, L = VB_diarization.VB_diarization(X, m, iE, w, V, sp=None, q=q, maxSpeakers=2, maxIters=10, VtiEV=VtiEV,
