...
def VB_diarization(X, m, iE, w, V, sp=None, q=None,
    ...
    if downsample is not None:
        ...
        downsampler = coo_matrix(
            (np.ones(nframes), (np.ceil(np.arange(nframes) / downsample).astype(np.int), np.arange(nframes))))
    ...
    for ii in range(maxIters):
        ...
        for sid in range(maxSpeakers):
            ...
            lls[:, sid] = G + VtiEF.dot(a) - 0.5 * NN.dot(
                mixture_sum.dot(((invL + np.outer(a, a)).astype(V.dtype).dot(V) * (iE.flat * V)).sum(0)))
