...
def uppertri_from_sym(covs_full, utr, utc):
    ...
    for ii in range(n_mix):
        covs_ut2d[:, ii] = covs_full[:, :, ii][(utr, utc)]
    return covs_ut2d
...
def gmm_eval_prep(weights, means, covs):
    ...
    if is_full_cov:
        ...
        for ii in range(n_mix):
            ...
            GMM['gconsts2'][ii] = - 0.5 * (logdetC + means[ii].dot(invCovMean) + dim * np.log(2.0 * np.pi))
        ...
        GMM['gconsts'] = np.log(weights) - 0.5 * (
                    np.sum(np.log(covs) + means ** 2 * GMM['invCovs'], axis=1) + dim * np.log(2.0 * np.pi))
        GMM['gconsts2'] = - 0.5 * (
                    np.sum(np.log(covs) + means ** 2 * GMM['invCovs'], axis=1) + dim * np.log(2.0 * np.pi))
